...
def KNN_Model(x, y, nbre_cv, k_max):
    ...
    for k in range(1,k_max+1):
        ...
        mean_score = np.mean(cross_val_score(model, x, y, cv=nbre_cv))
        print("---")
        print(f"accuracy moyenne pour k = {k}: { mean_score}")
        print("---")
        scores.append(mean_score)
    
    print("cross validation terminée")
    
    print("--- Plot de l'accuracy en fonction de K ---")
    
    abscisse = [i for i in range(1, len(scores)+1)]
    
    plt.plot(abscisse, scores, label="Accuracy en fonction de K")
    plt.show()
